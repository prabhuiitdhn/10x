"""
Perception Assignment — Solution
=================================
...
# assumed RealSense D435 typical intrinsics for 640×480 depth stream
FX, FY = 381.0, 381.0        # focal lengths (pixels)
CX, CY = 320.0, 240.0        # principal point (pixels)

# RANSAC parameters
RANSAC_ITERS   = 300          # number of random trials
RANSAC_THRESH  = 0.025        # inlier distance threshold (metres)
...
with Reader(BAG_DIR) as reader:
    ...
    for conn, ts, raw in reader.messages(connections=conns):
        ...
        frames.append(depth);  timestamps.append(ts)
...
for i, (depth, ts) in enumerate(zip(frames, timestamps)):
    ...
    u_ax = Vt2[0];  v_ax = np.cross(normal, u_ax)
    ...
    axes[0, i].set_title(f'F{i+1}  +{t:.1f}s', fontsize=8); axes[0, i].axis('off')
...
# ══════════════════════════════════════════════════════════════════════════════
# When a rigid body rotates about a fixed axis, its face normal traces a circle
# on the unit sphere.  The rotation axis is perpendicular to that circle's plane.
...
print("\n" + "=" * 60)
print("OUTPUTS SAVED")
print(f"  output/results_table.txt")
print(f"  output/rotation_axis.txt")
print(f"  output/result_plot.png")
